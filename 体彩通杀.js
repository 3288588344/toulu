 ...
 */
// ============================================================================================================
const $ = new Env('vx体彩服务号') 
const axios = require('axios')
const env_name = 'gdtc' //环境变量名字
...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/gdtc.js");
    await main();//主函数
...
async function SignTask(user) {
    try {
        let urlObject = {
            method: 'post',
            url: 'https://pnup-hd.tcssyw.com/api/act/do_sign_in',
            headers: {
                'Host': 'pnup-hd.tcssyw.com',
        ...
        if (result?.ret == "0") {
            //打印签到结果
            DoubleLog(`🌸账号[${user.index}]` + `🕊签到成功，获得[${result.data.getwealth}]积分,总积分:${result.data.allwealth}🎉`);
        }if(result?.ret == "10005") {
...
async function account(user) {
    try {
        let urlObject = {
            ...
            headers: {
                'Host': 'pnup-fj.lottery-sports.com',
                'Content-Type': 'application/x-www-form-urlencoded',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) XWEB/9129',
            },
            data: {
        ...
        //console.log(result);
        if (result?.code == "0") {
            DoubleLog(`🌸账号[${user.index}]🕊账户当前积分${result.data.records[0].postMemberPoints}💰`)
